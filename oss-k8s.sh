...
MAIN_TF_BACKUP="$WORKDIR/.main.tf.iac-backup"
RUNTIME_STUB_TF="$WORKDIR/runtime-provider-stubs.tf"
...
write_provider_config() {
  if [[ "$SYSTEM" == "openstack" ]]; then
    cat > "$WORKDIR/provider.tf" << 'PROVIDEREOF'
# Copyright © 2022-2024, SAS Institute Inc., Cary, NC, USA. All Rights Reserved.
# SPDX-License-Identifier: Apache-2.0
...
terraform_up() {
    terraform_prep
    terraform apply -parallelism=20 -state $TFSTATE -auto-approve "${TF_COMPAT_ARGS[@]}" -var "deployment_type=$SYSTEM" -var-file $TFVARS
...
patch_vip_allowed_pairs() {
    local VIP
    VIP=$(grep -E '^\s*cluster_vip_ip\s*=' "$TFVARS" 2>/dev/null | head -1 | sed 's/.*=\s*"\(.*\)".*/\1/' | tr -d ' "')
    if [[ -z "$VIP" ]]; then
...
" 2>/dev/null)
    ...
    local PATCH_BODY="{\"port\":{\"allowed_address_pairs\":[{\"ip_address\":\"${VIP}\"}]}}"

    echo "patch_vip_allowed_pairs: patching control-plane ports for cluster '${CLUSTER_NAME}' with VIP ${VIP}"
...
terraform_down() {
    terraform_prep
    terraform destroy -parallelism=20 -state $TFSTATE -auto-approve "${TF_COMPAT_ARGS[@]}" -var "deployment_type=$SYSTEM" -var-file $TFVARS
}
