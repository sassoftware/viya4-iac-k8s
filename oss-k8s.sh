...
gather_tf_creds() {
    ...
    if [[ "$SYSTEM" == "azure" ]]; then
      # Prefer environment AZURE_* vars if provided (common for Azure tooling)
      export TF_VAR_azure_subscription_id=${AZURE_SUBSCRIPTION_ID:-$TF_VAR_azure_subscription_id}
      export TF_VAR_azure_tenant_id=${AZURE_TENANT_ID:-$TF_VAR_azure_tenant_id}
      export TF_VAR_azure_client_id=${AZURE_CLIENT_ID:-$TF_VAR_azure_client_id}
      export TF_VAR_azure_client_secret=${AZURE_CLIENT_SECRET:-$TF_VAR_azure_client_secret}
      export TF_VAR_azure_use_msi=${AZURE_USE_MSI:-$TF_VAR_azure_use_msi}
    fi
...
# Copyright © 2022-2024, SAS Institute Inc., Cary, NC, USA. All Rights Reserved.
# SPDX-License-Identifier: Apache-2.0
# Auto-generated by oss-k8s.sh for SYSTEM=azure -- do not edit manually.
...
# Emits a modules/noop stand-in for a for_each-style module (control_plane,
# system, node and their os_*/az_* equivalents) so the cross-mode references
# in locals.tf keep resolving when the real module block is stripped out of
# main.tf for the active SYSTEM. All modules/noop variables have defaults.
stub_multi_module() {
...
# Emits a modules/noop stand-in for a single-instance module (jump/nfs/cr/
# api_lb style servers) referenced by locals.tf or main.tf outside the active
# SYSTEM's own topology.
stub_single_module() {
  cat >> "$RUNTIME_STUB_TF" << STUBEOF
module "$1" {
  source = "./modules/noop"
...
write_main_config() {
  ...
  if [[ "$SYSTEM" == "openstack" ]]; then
    # Keep only the OpenStack node modules; strip vSphere and Azure entirely.
    awk '
      /^# vSphere – provider data sources/ { skip = 1; next }
      /^# OpenStack – Kubernetes node modules/ { skip = 0 }
      /^## Azure – Kubernetes node modules/ { skip = 1; next }
      /^# Ansible inventory \+ vars files/ { skip = 0 }
      !skip { print }
    ' "$MAIN_TF_BACKUP" > "$MAIN_TF"

    stub_multi_module control_plane
    stub_multi_module system
    stub_multi_module node
    stub_multi_module az_control_plane
    stub_multi_module az_system
    stub_multi_module az_node
    stub_single_module az_jump
    stub_single_module az_nfs
    stub_single_module az_cr
    stub_single_module az_api_lb
  elif [[ "$SYSTEM" == "vsphere" ]]; then
    # Keep only the vSphere node modules; strip OpenStack and Azure entirely.
    awk '
    ...
    stub_multi_module os_control_plane
    stub_multi_module os_system
    stub_multi_module os_node
    stub_single_module os_jump
    stub_single_module os_nfs
    stub_single_module os_cr
    stub_multi_module az_control_plane
    stub_multi_module az_system
    stub_multi_module az_node
    stub_single_module az_jump
    ...
    # Keep only the Azure node modules; strip vSphere and OpenStack entirely.
    awk '
      /^# vSphere – provider data sources/ { skip = 1; next }
      /^## Azure – Kubernetes node modules/ { skip = 0 }
      !skip { print }
    ' "$MAIN_TF_BACKUP" > "$MAIN_TF"
