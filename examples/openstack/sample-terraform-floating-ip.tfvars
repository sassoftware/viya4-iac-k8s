...
#    ]
#
#  NOTE: Each Kubernetes LoadBalancer-type Service consumes one IP from this pool.
#        When using viya4-deployment, allocate enough IPs for every LB you plan to enable:
#          - 1 IP  (always)                              Envoy (Contour) — main Viya HTTPS endpoint
#          - 1 IP  (V4_CFG_CAS_ENABLE_LOADBALANCER: true) CAS binary-port LB
#          - 1 IP  (CAS Connect LB, if enabled)
#          - 1 IP  (Consul LB, if enabled)
#        On OpenStack, use allocate-vip.sh which defaults to LB_COUNT=5.
#        If the pool runs out a Service stays Pending and pods (e.g. CAS) fail to start.
cluster_lb_addresses = []
