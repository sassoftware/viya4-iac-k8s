...
# Rule 2a: Kubernetes API Server (Port 6443) - Allow from approved source CIDRs
# NOTE: when using source_address_prefixes (plural), service tags must go in a separate rule.
resource "azurerm_network_security_rule" "k8s_api" {
  ...
  description                 = "Allow Kubernetes API server access from approved CIDRs and Azure LB health probes"
  priority                    = 110
  direction                   = "Inbound"
  access                      = "Allow"
  protocol                    = "Tcp"
  ...
  # Include user-approved CIDRs. Azure LB probe source (AzureLoadBalancer service tag) goes in a separate rule.
  source_address_prefixes     = var.api_server_source_cidrs
  destination_address_prefix  = "*"
  resource_group_name         = var.resource_group_name
  network_security_group_name = local.nsg_name
...
# Rule 2b: Allow Azure Load Balancer health probes to reach the API server (Port 6443)
# Without this, the LB probe fails and the backend is marked unhealthy.
resource "azurerm_network_security_rule" "k8s_api_lb_probe" {
  count                       = var.create_nsg_rules ? 1 : 0
  name                        = "AllowKubernetesAPIFromAzureLB"
  description                 = "Allow Azure LB health probes (168.63.129.16) to reach kube-apiserver on 6443"
  priority                    = 111
  direction                   = "Inbound"
  access                      = "Allow"
  protocol                    = "Tcp"
