...
Automates provisioning and configuration of production-grade Kubernetes clusters for SAS Viya 4 deployments across **OpenStack (HPOS)**, **VMware vSphere**, **Microsoft Azure**, and **Bare Metal** environments.
...
- [Step 1 — Copy the sample tfvars for your topology](#step-1--copy-the-sample-tfvars-for-your-topology)
- [Step 2 — Edit terraform.tfvars](#step-2--edit-terraformtfvars)
- [Step 3 — Set credentials and run](#step-3--set-credentials-and-run)
...
This repository implements the **topology dispatcher pattern**: a single codebase that supports four platforms without any shared mutable state between them.

Each platform lives in its own fully self-contained directory under `topologies/`, with its own Terraform provider, variables, Ansible playbooks, and OS-level roles. A thin root `main.tf` acts as a dispatcher but is **not invoked by the scripts**. Both `deploy.sh` and `oss-k8s.sh` call Terraform with `-chdir=topologies/<system>` directly, so only the active topology's provider is ever loaded or authenticated.

Shared Kubernetes bootstrap logic, including kubeadm, CNI, storage, and metrics, lives in `roles/kubernetes/` at the repository root and is inherited by all topologies through the following setting in each topology's `ansible.cfg`:
...
| --- | --- | --- |
| `apply` | Terraform | Provisions VMs, disks, and networks, then generates `inventory` and `ansible-vars.yaml` |
| `setup` | Ansible | Installs OS packages, configures sysctl settings, SSH keys, containerd, and Helm |
| `install` | Ansible | Runs `kubeadm init`, installs Calico CNI and kube-vip, joins nodes, and fetches kubeconfig |

After `apply setup install` completes, a ready-to-use kubeconfig is written to the workspace directory. Use that kubeconfig immediately with `kubectl`.
...
**Note:** Root `main.tf` exists as an alternative entry point for direct `terraform apply` from the repository root, but it is **not invoked by the scripts**. Scripts call Terraform with `-chdir=topologies/<system>` directly so only the active topology's provider is loaded and authenticated.
...
Each topology under `topologies/` is a **fully self-contained Terraform root module** with its own `provider.tf`, `variables.tf`, `playbooks/`, and `roles/systems/`.

The shared `roles/kubernetes/` directory at the repository root is used by all topologies through the following setting in each topology's `ansible.cfg`:
...
| Topology | Script | Terraform Directory | VMs Provisioned |
| --- | --- | --- | --- |
...
| `bare_metal` | `scripts/oss-k8s.sh` | None; Ansible only | Pre-existing machines |

### Prerequisites

Use of these tools requires operational knowledge of Terraform, Ansible, Kubernetes, SSH key management, and the target infrastructure platform.
...
| Terraform | >= 1.10.0 |
| Python 3 | >= 3.8 |
| ansible-core | 2.16.4, installed through `requirements.txt` |
...
Ansible uses SSH key authentication to connect to all cluster nodes. Generate a dedicated keypair:
...
Upload `~/.ssh/oss/my-keypair.pub` to your cloud platform, then set `openstack_ssh_keypair` or `control_plane_ssh_key_name` in `terraform.tfvars` to match the uploaded keypair name.

### Quick Start

When you have prepared the prerequisites for your target platform, you are ready to obtain and customize the input values that will set up your Kubernetes cluster.
...
- `prefix` — cluster name prefix. The cluster becomes `<prefix>-oss`.
- `deployment_type` — `openstack`, `vsphere`, `azure`, or `bare_metal`.
- Cloud-specific settings, including image, keypair, network, and flavors or sizes.
...
| --- | --- | --- |
| `terraform.tfstate` | Repository root | Terraform state; keep safe and back up |
| `inventory` | Repository root | Ansible inventory with node IPs and groups |
| `ansible-vars.yaml` | Repository root | Ansible variables passed to all playbooks |
| `<prefix>-oss-kubeconfig.conf` | Repository root | Cluster admin kubeconfig |
...
- `uninstall` — runs `kubernetes-uninstall.yaml` and removes kubeadm state from all nodes.
- `destroy` — runs `terraform destroy`, deletes all cloud resources, and removes state files.

You will be prompted to confirm with `yes` before `destroy` proceeds.

### Docker Usage

The Docker image has all tooling pre-installed, including Terraform, Ansible, Helm, `kubectl`, and the OpenStack CLI. Mount your workspace directory containing `terraform.tfvars`:
...
The entrypoint automatically routes `SYSTEM=azure` to `deploy.sh` and all other values to `oss-k8s.sh`. All files, including `terraform.tfvars`, `terraform.tfstate`, `inventory`, `ansible-vars.yaml`, and kubeconfig, are read from and written to `/workspace`.
