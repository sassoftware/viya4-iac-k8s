...
# vsphere data sources and module calls live in vsphere_compute.tf.disabled.
# oss-k8s.sh activates that file (copies to vsphere_compute.tf) only when
# DEPLOYMENT_TYPE=vsphere, keeping azure/bare_metal completely free of any
# vsphere resource types and therefore free of vsphere provider initialisation.
