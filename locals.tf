...
locals {
  ...
  node_pools  = var.node_pools == null ? tomap({}) : { for k, v in var.node_pools : k => merge(var.node_pool_defaults, v) }
  node_labels = { for k, v in local.node_pools : k => [for lk, lv in try(v.node_labels, {}) : "${lk}=${lv}"] }
  node_taints = { for k, v in local.node_pools : k => try(v.node_taints, []) }

  ## Control plane nodes
  control_plane_nodes = local.node_pools == null ? tomap({}) : { for k, v in local.node_pools : k => v if k == "control_plane" }

  # IPs for control-plane nodes – prefer OpenStack output, then Azure, else vSphere
  ...
  ) : var.deployment_type == "azure" ? (
    flatten(sort(flatten([for item in values(try(module.az_control_plane, {})) : [try(item.private_ip_address, null)]])))
  ) : (
    flatten(sort(flatten([for item in values(try(module.control_plane, {})) : values(item)])))
  )

  ## System nodes
  system_nodes = local.node_pools == null ? tomap({}) : { for k, v in local.node_pools : k => v if k == "system" }

  ## Nodes (all non-control-plane, non-system pools)
  nodes = local.node_pools == null ? tomap({}) : { for k, v in local.node_pools : k => v if(k != "control_plane" && k != "system") }

  # Worker node IPs – prefer OpenStack output, then Azure, else vSphere
  node_ips = var.deployment_type == "openstack" ? (
    ...
    flatten(sort(flatten([for item in values(merge(try(module.az_system, {}), try(module.az_node, {}))) : [try(item.private_ip_address, null)]])))
  ) : (
  ...
  resolved_jump_ip = (
    ...
    ) : var.deployment_type == "azure" && var.create_jump ? (
      try(module.az_jump[0].public_ip_address, null)
    ) : (
  ...
  resolved_nfs_ip = (
    ...
    ) : var.deployment_type == "azure" && var.create_nfs ? (
      try(module.az_nfs[0].public_ip_address, null)
    ) : (
  ...
  resolved_cr_ip = (
    ...
    ) : var.deployment_type == "azure" && var.create_cr ? (
      try(module.az_cr[0].public_ip_address, null)
    ) : (
  ...
  # PostgreSQL
  postgres_servers = var.postgres_servers == null ? tomap({}) : { for k, v in var.postgres_servers : k => merge(var.postgres_server_defaults, v) }
